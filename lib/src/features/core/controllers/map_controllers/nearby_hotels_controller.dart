...
class NearbyHotelsController extends GetxController {
  //Creating instance for the Nearby hotels
  static NearbyHotelsController get instance => Get.find();
  ...
  Future<List<Marker>> getHotelMarkers(
      double lat, double lng, double distance) async {
    List<Marker> markers = [];

    // Fetch hotel data from OpenStreetMap
    var response = await http.get(Uri.parse(
        'https://overpass-api.de/api/interpreter?data=[out:json][timeout:25];node[tourism=hotel](${lat - 0.1},${lng - 0.1},${lat + 0.1},${lng + 0.1});out;'));
