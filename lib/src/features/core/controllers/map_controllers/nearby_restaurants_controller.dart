...
class NearbyRestaurantsController extends GetxController {
  //Creating instance for the Nearby Restaurants
  static NearbyRestaurantsController get instance => Get.find();
  ...
  Future<List<Marker>> getRestaurantMarkers(
      double lat, double lng, double distance) async {
    List<Marker> markers = [];

    // Fetch hotel data from OpenStreetMap
    var response = await http.get(Uri.parse(
        'https://overpass-api.de/api/interpreter?data=[out:json][timeout:25];node[amenity=restaurant](${lat - 0.1},${lng - 0.1},${lat + 0.1},${lng + 0.1});out;'));
